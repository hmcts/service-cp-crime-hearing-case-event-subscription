...
AMP-504 introduces payload storage and a new retrieval endpoint for PCR events. To allow safe incremental delivery, all new behaviour is gated behind `NOTIFICATION_JSON_ENABLED`. This design covers only the toggle wiring; subsequent changes will add the actual storage and endpoint logic behind it.
...
**Goals:**
- Add `notification.json.enabled` Spring property backed by `NOTIFICATION_JSON_ENABLED` env var (default `false`)
