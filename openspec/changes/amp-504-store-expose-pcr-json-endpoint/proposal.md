...
- `@JdbcTypeCode(SqlTypes.JSON)` on `HearingEventPayloadEntity.rawPayload` for JSONB serialisation (Hibernate 6 — no separate converter class)
...
- Unit tests: `HearingEventPayloadServiceTest`, extended `CallbackDeliveryServiceTest`
...
- **New source files**: `HearingEventPayloadEntity`, `HearingEventSubscriptionEntity`, `HearingEventPayloadRepository`, `HearingEventSubscriptionRepository`, `HearingEventPayloadService`
...
- **Test files**: `HearingEventPayloadServiceTest`, `HearingEventPayloadRepositoryTest`, `HearingEventSubscriptionRepositoryTest`; `CallbackDeliveryServiceTest` extended with 5 toggle-gated scenarios; `FlywayMigrationIntegrityTest` and `IntegrationTestBase` updated
