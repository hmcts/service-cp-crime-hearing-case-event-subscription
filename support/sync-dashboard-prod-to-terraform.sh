#!/usr/bin/env bash
# Syncs dashboard-kql-prod/ to the cp-amp-terraform-az-dashboard queries-prod/hearing-results-document-subscription/ folder.
# Run from anywhere — uses the script's own location to find both repos.

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
SRC="$SCRIPT_DIR/dashboard-kql-prod"
...
DEST="$TF_REPO/queries-prod/hearing-results-document-subscription"
